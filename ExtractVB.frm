...
Private Function CountSubString(vTest As Variant, _
                                vFind As Variant) As Long

  CountSubString = UBound(Split(vTest, vFind))
    ...
    If FolderExist(sFolder) Then
      ...
      For I = 0 To (nInCount - 1)
        ...
        For J = LBound(arrHidden) To UBound(arrHidden)
          ...
          ' ProgressBar J / UBound(arrHidden) * 100 'IIf(nProgress > 100, 100, nProgress)
          sString = arrHidden(J)
          ...
          If LenB(sImageData) Then
            'ProgressBar J / UBound(arrHidden) * 100
...
Private Sub GenerateNewThumb(ByVal nImageCount As Long, _
  ...
  End With
  'PositionThumbs
  'picThumbnail.Left = HScroll.Width - imgThumbnail.Count * 460
  If picThumbnail.Width > HScroll.Width Then
    picThumbnail.Left = HScroll.Width - (imgThumbnail.Count - 1) * 460
...
Private Sub GetHiddenTxt(ByVal strFilename As String, _
  ...
  MaxPossible = CountSubString(strData, ":")
...
Private Function GetImageExtention(ByVal sImageData As String) As String
  ...
  If Left$(sImageData, 3) = "GIF" Then
   ...
   ElseIf Mid$(sImageData, 6, 5) = "ºExif" Then
    GetImageExtention = "jpg" 'Or jpeg
   ElseIf Left$(sImageData, 4) = "Œ-„‹" Then
...
Private Sub Guage(pic As Control, _
  ...
  With pic
    .DrawMode = vbCopyPen
    pic.Line (iRight, iLineWidth)-(iRight, iBottom), vbWhite, BF
    pic.Line (iLineWidth, iBottom)-(iRight, iBottom), vbWhite, BF
...
Private Sub HScroll_Change()
  ...
  picThumbnail.Left = -(HScroll.Value * 460)
...
Private Sub PositionThumbs()
  ...
  If 460 * (pos - 1) > HScroll.Width Then
    picThumbnail.Width = 460 * pos '(pos - 1)
   Else
    picThumbnail.Width = HScroll.Width
  End If
  HScroll.Enabled = picThumbnail.Width > HScroll.Width
  With HScroll
    .Max = imgThumbnail.Count '(picThumbnail.Width - .Width)
    If .Value > .Max Then
      .Value = .Max
    End If
  End With
...
Private Sub ProcessOneImage(sImageData As String, _
  ...
  ProgressBar imgThumbnail.Count / MaxPossible * 100
...
':)Code Fixer V2.9.6 (8/02/2005 11:19:25 AM) 23 + 928 = 951 Lines Thanks Ulli for inspiration and lots of code.
